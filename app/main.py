...
class ResultsPage(QWidget):
    ...
    def export_pdf(self) -> None:
        if not self._current_result:
            QMessageBox.information(self, "No Result", "There is no analysis result to export.")
            return
            
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Save PDF Report", "Breast_Tumor_Analysis_Report.pdf", "PDF Files (*.pdf)"
        )
        if not file_path:
            return
        ...
        try:
            ...
            font.setBold(True)
            painter.setFont(font)
            painter.drawText(200, 400, "Breast Tumor Analysis Report")
            ...
            painter.drawText(200, y_offset, f"File Analyzed: {Path(self._current_result.image_path).name}")
            y_offset += 200
            
            # Summary Text
            for line in self._current_result.summary_text.split('\n'):
                painter.drawText(200, y_offset, line)
                y_offset += 150
                
            y_offset += 200
            
            # Draw currently visible images
            images = [
                ("Original Scan", self.original_panel.get_display_pixmap()),
                ("Segmentation Mask", self.segmentation_panel.get_display_pixmap()),
                ("Grad-CAM Overlay", self.gradcam_panel.get_display_pixmap())
            ]
            ...
            for title, pixmap in images:
                if pixmap and not pixmap.isNull():
                    ...
                    font.setBold(True)
                    painter.setFont(font)
                    painter.drawText(200, y_offset, title)
                    y_offset += 200
                    
                    target_width = writer.width() - 400
                    scaled = pixmap.scaledToWidth(target_width, Qt.TransformationMode.SmoothTransformation)
                    
                    if y_offset + scaled.height() > writer.height() - 400:
                        writer.newPage()
                        y_offset = 400
                        painter.drawText(200, y_offset, title + " (Continued)")
                        y_offset += 200
                        
                    painter.drawPixmap(200, y_offset, scaled)
                    y_offset += scaled.height() + 400
            ...
            font.setBold(False)
            painter.setFont(font)
            painter.drawText(200, writer.height() - 300, "Disclaimer: This report is generated by an AI decision support prototype. Final interpretation should be reviewed by a qualified clinician.")
            
            painter.end()
            QMessageBox.information(self, "Export Successful", f"PDF report successfully saved to:\n{file_path}")
        except Exception as e:
            logger.error(f"Failed to export PDF: {e}")
            QMessageBox.warning(self, "Export Failed", f"An error occurred while saving the PDF:\n{e}")
